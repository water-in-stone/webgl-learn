...
import vertexShaderSource from "../shaders/transformation/3d-camera-light/vertex-2d.glsl";
import fragmentShaderSource from "../shaders/transformation/3d-camera-light/fragment-2d.glsl";
...
export default class App {
    ...
    render() {
        ...
        const drawScene = () => {
            ...
            gl.vertexAttribPointer(
                positionLocation, size, type, normalized, stride, offset);
            ...
            var normalized = false; // normalize the data (convert from 0-255 to 0-1)
            var stride = 0;        // 0 = move forward size * sizeof(type) each iteration to get the next position
            var offset = 0;        // start at the beginning of the buffer
            gl.vertexAttribPointer(
                normalLocation, size, type, normalized, stride, offset);

            // Compute the projection matrix
            var aspect = (gl.canvas as HTMLCanvasElement).clientWidth / (gl.canvas as HTMLCanvasElement).clientHeight;
            var zNear = 1;
            var zFar = 2000;
            var projectionMatrix = this.m4.perspective(fieldOfViewRadians, aspect, zNear, zFar);

            // Compute the camera's matrix
            var camera = [100, 150, 200];
            var target = [0, 35, 0];
            var up = [0, 1, 0];
            var cameraMatrix = this.m4.lookAt(camera, target, up);
            ...
            var worldMatrix = this.m4.yRotation(fRotationRadians);

            // Multiply the matrices.
            var worldViewProjectionMatrix = this.m4.multiply(viewProjectionMatrix, worldMatrix);
            ...
            //! 这是一个三维向量，其中每个分量表示光源在x、y和z轴上的方向分量。
    ...
    setGeometry(gl) {
        ...
        var matrix = this.m4.xRotation(Math.PI);
        matrix = this.m4.translate(matrix, -50, -75, -15);
